...
public class Controller implements Initializable {
            ...
            for (int ox = -1; ox < 2; ox++) {
                for (int oy = -1; oy < 2; oy++) {
                    if (checkInsideArray(x + ox, y + oy)) {
                        if (buttonsCurrentPlayer[x + ox][y + oy].getText().equals("X") && buttonsCurrentPlayer[x + ox][y + oy] != buttonsCurrentPlayer[x][y]) {
                            answer = true;
            ...
            for (int i = -1; i < 2; i++) {
                for (int j = -1; j < 2; j++) {
                    if (checkInsideArray(x + i, y + j)) {
                        if (!buttonsCurrentPlayer[x + i][y + j].getText().equals("X")) {
                            setGraphicOnButton(false ,buttonsCurrentPlayer[x + i][y + j]);
    ...
    // установка картинки моря на кнопку не корабля
    private void setGraphicOnButton(boolean cancel, Button btn) {
        if (!cancel)
            btn.setGraphic(new ImageView(new Image("sea.jpg")));
        else
            btn.setGraphic(null);
    }
            ...
            for (int i = -1; i < 2; i++) {
                for (int j = -1; j < 2; j++) {
                    if (checkInsideArray(x + i, y + j) && !buttonsCurrentPlayer[x + i][y + j].getText().equals("")) {
                        setGraphicOnButton(true, buttonsCurrentPlayer[x + i][y + j]);
    ...
    private void setDisableFalseToButtons() {
        for (int x = 0; x < 10; x++) {
            for (int y = 0; y < 10; y++) {
                setGraphicOnButton(true, buttonsCurrentPlayer[x][y]);
    ...
    private void shootingShip() {
        if (currentPressedBtn.getText().equals("X")) { // попадание
            ...
            setGraphicOnButton(false, currentPressedBtn);
    ...
    private boolean isAroundNotKilledShips() {
        ...
        for (Button btn : listButtonsKilledShipsCurrentPlayer) {
            ...
            for (int ox = -1; ox < 2; ox++) {
                for (int oy = -1; oy < 2; oy++) {
                    if (checkInsideArray(x + ox, y + oy)) {
                        if (buttonsCurrentPlayer[x + ox][y + oy].getText().equals("X") && buttonsCurrentPlayer[x + ox][y + oy].getStyle().equals("-fx-background-color: #eeeeee"))
                            answer = true;
                    }
                }
            }
    ...
    private void paintAroundKilledShips(Button btn) {
        ...
        for (int ox = -1; ox < 2; ox++) {
            for (int oy = -1; oy < 2; oy++) {
                if (checkInsideArray(x + ox, y + oy)) {
                    if (!buttonsCurrentPlayer[x + ox][y + oy].getText().equals("X")) {
                        setGraphicOnButton(false, buttonsCurrentPlayer[x + ox][y + oy]);
    ...
    private void paintUndetectedShips() {
        for (int x = 0; x < 10; x++) {
            for (int y = 0; y < 10; y++) {
                if (buttonsPlayer1[x][y].getStyle().equals("-fx-background-color: #eeeeee") && buttonsPlayer1[x][y].getText().equals("X")) {
                    buttonsPlayer1[x][y].setStyle("-fx-background-color: #c8a415");
                    buttonsPlayer1[x][y].setTextFill(Color.web("#fafafa"));
                }
                if (buttonsPlayer2[x][y].getStyle().equals("-fx-background-color: #eeeeee") && buttonsPlayer2[x][y].getText().equals("X")) {
    ...
    public void onNewGame(ActionEvent event) {
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                setGraphicOnButton(true, buttonsPlayer1[i][j]);
                setGraphicOnButton(true, buttonsPlayer2[i][j]);
